...
import { Plus, Search, ExternalLink, CalendarDays, UserX, Pencil, Trash2 } from "lucide-react";
...
export default function FreeAgentsView() {
  ...
  const { data: players = [], isLoading } = useQuery({
    queryKey: ['freeAgentPlayers'],
    queryFn: async () => {
      const user = await base44.auth.me();
      const all = await base44.entities.Player.list('-created_date');
      return all.filter(p => p.agency_id === user.agency_id && p.player_type === 'free_agent' && !p.archive_id);
    },
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['freeAgentPlayers'] });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Spieler ohne aktuellen Verein – {filtered.length} Spieler
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            Vereinslosen Spieler hinzufügen
          </Button>
        </div>
      </div>
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-xl p-4 flex items-start gap-3">
        <UserX className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
        <div>
          <p className="text-sm font-semibold text-teal-800">Was ist die Liste vereinsloser Spieler?</p>
          <p className="text-sm text-teal-700 mt-0.5">
            Hier pflegen Sie Spieler, die aktuell keinen Verein haben und einen neuen Club suchen.
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-lg font-medium">Keine vereinslosen Spieler</p>
          <p className="text-slate-500 text-sm mt-1">Fügen Sie Spieler hinzu, die aktuell keinen Verein haben.</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence>
            {filtered.map(player => (
              <motion.div key={player.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                <Card className="hover:shadow-xl transition-all duration-200 bg-white dark:bg-slate-900 relative rounded-xl overflow-hidden border-2 border-teal-200 dark:border-teal-800 group">
                  {/* Teal accent bar */}
                  ...
                  >
                    <CardHeader className="pb-2 pt-3 px-4">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        <Badge variant="outline" className="border-teal-200 bg-teal-50 text-teal-800 font-semibold text-xs">{player.position}</Badge>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <FreeAgentForm player={newPlayer} onChange={setNewPlayer} positions={POSITIONS} />
      ...
      <Dialog open={!!editingPlayer} onOpenChange={() => setEditingPlayer(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Vereinslosen Spieler bearbeiten</DialogTitle>
          </DialogHeader>
          {editingPlayer && (
            <>
              <FreeAgentForm player={editingPlayer} onChange={setEditingPlayer} positions={POSITIONS} />
              <div className="flex justify-end gap-3 pt-2">
      ...
      <Dialog open={!!deleteCandidate} onOpenChange={() => setDeleteCandidate(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-slate-600 py-2">
            Soll <strong>{deleteCandidate?.name}</strong> aus der Liste vereinsloser Spieler entfernt werden?
          </p>
...
function FreeAgentForm({ player, onChange, positions }) {
  return (
    <div className="space-y-4 py-2">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          <Label>Notizen</Label>
          <Textarea value={player.notes || ""} onChange={e => onChange({ ...player, notes: e.target.value })} placeholder="Hintergrundinfos, letzter Verein, Ansprechpartner..." className="mt-1.5 h-24" />
        </div>
      </div>
    </div>
  );
